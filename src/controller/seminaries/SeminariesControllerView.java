...
public class SeminariesControllerView extends HttpServlet {
		...
		if(uGoogle == null){
			...
			if(uSearch.isEmpty()){
				...
				if( rSearch.isEmpty()){
					...
					if(aSearch.isEmpty()){
						error="El usuario logueado no tiene acceso a las vistas";
						request.setAttribute("error", error);
						RequestDispatcher dp = getServletContext().getRequestDispatcher("/WEB-INF/Views/Errors/error.jsp");
						dp.forward(request, response);
					}else{
						...
						try{
							...
							view=pm.getObjectById(Seminary.class, ksem);
							request.setAttribute("vista", view);
						}finally{
							pm.close();
						}			
						RequestDispatcher dispatcher = getServletContext().getRequestDispatcher("/WEB-INF/Views/Seminaries/view.jsp");
